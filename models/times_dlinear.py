    ...
    def forward(self, x):
        ...
        for i in range(self.k):
            ...
            if (self.seq_len + self.pred_len) % period != 0:#如果不能整除
                ...
                padding = torch.zeros([x.shape[0], (length - (self.seq_len + self.pred_len)), x.shape[2]]).to(x.device)#(length - (self.seq_len + self.pred_len)是补了多少0
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        x = dec_out + trend_output.permute(0, 2, 1)
