...
class TimesBlock1(nn.Module):
    ...
    def forward(self, Season,Trend):
        ...
        period_list = [self.cutpos+1,self.cutpos+5,self.cutpos+10]
        ...
        # Trend_out_res =  Trend
        ######注意力token切分
        # period_list = [self.cutpos + 1, self.cutpos + 20, self.cutpos + 10]
...
class TimesBlock2(nn.Module):
    def __init__(self, configs):
        ...
        self.encoder_season = TransformerEncoder(configs.d_model, configs.hidden_size, configs.num_layers, configs.num_heads, configs.dropout)
        self.encoder_trend = TransformerEncoder(configs.d_model, configs.hidden_size, configs.num_layers,configs.num_heads, configs.dropout)
        # self.encoder_season = TransformerEncoder(configs.input_size2, configs.hidden_size, configs.num_layers,
        #                                          configs.num_heads, configs.dropout)
        # self.encoder_trend = TransformerEncoder(configs.input_size2, configs.hidden_size, configs.num_layers,
        #                                         configs.num_heads, configs.dropout)
    def forward(self, Season,Trend):
        # B, T, N = Season.size()
        ...
        # Season_out_att = self.encoder_season(Season_out,Trend_out,Season_out)
        # Trend_out_att = self.encoder_season(Trend_out, Season_out,Trend_out)
        Season_out_att = self.encoder_season(Season, Trend, Season)
        Trend_out_att = self.encoder_season(Trend, Season, Trend)
        Season_out = Season_out_att[:, :(self.seq_len + self.pred_len), :]
        Trend_out = Trend_out_att[:, :(self.seq_len + self.pred_len), :]
        return Season_out , Trend_out
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.model = nn.ModuleList([TimesBlock1(configs),TimesBlock2(configs),TimesBlock3(configs)])#,TimesBlock2(configs)
